...
from SynergyV3000 import api, username, ov_pw, user, cim_pw, old_pw
...
def enclosure_inventory(ip, auth):
    ...
    enc_list = printDict(EncDict, ['name', 'version', 'state', 'enclosureType'])
    ...
    for i, value in enumerate(enc_list):
        ...
        enc_type =  enc_list[i]['enclosureType']
        ...
        logging.testcases("The type of enclosure: {}".format(enc_type))
        ...
        for k, value in enumerate(cim_bays):
            ...
            logging.testcases("Composer Type: {}" .format(cim_model))
    ...
    logging.testcases("#################################INTERCONNECTS#########################################")
